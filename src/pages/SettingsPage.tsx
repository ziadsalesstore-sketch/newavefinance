...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (data) setMode(data.sales_tracking_mode);
  }, [data]);
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("settings").upsert({
      user_id: user.id, sales_tracking_mode: mode, updated_at: new Date().toISOString(),
    });
    setBusy(false);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">System preferences. Financial values live in the <strong>Opening Balance</strong> section.</p>
      </div>
